...
Run `npm run build`, then open [`design-system.html`](design-system.html) in any browser. No server, no framework, no dependencies. **Start here if you are new to this library.** It walks the theming contract with live, switchable examples, shows every token as a swatch computed from the page's own resolved styles (never hand-typed, so it can't drift), a side-by-side gallery of all nine palettes, and a dedicated "Common mistakes" section with live before/after contrast comparisons and a try-it-yourself contrast checker. It is also served inside ConjureOS itself: launcher &rarr; **Design System**, next to Developer Docs.

For a denser, no-frills component reference once you already know the system, [`demo.html`](demo.html) renders every primitive in all nine themes and both flavors, with the markup for each beside it in a collapsible block. Both pages load the real `dist/ui.css`, so what you see is exactly what your app gets; anything wrong on either page is wrong in the package.

## Style guide

[MODERN_WHIMSY.md](MODERN_WHIMSY.md) is the canonical reference for the visual language: the theming contract, all nine palettes with their values, the full token reference, common mistakes (read this if something looks subtly wrong and you cannot tell why), every component with its markup, surface idioms, voice, and known drift. The final `## For agents` section is inline-imported by the ConjureOS Dev agent's system prompt, so edits to that section flow straight into AI-generated apps.
...
Produces `dist/ui.css`, a single concatenated stylesheet with a version header, `dist/theme.js`, the optional theme resolver, and `dist/design-system.html`, the interactive reference page with its stylesheet link rewritten to match where it actually ships. It also refreshes the autogenerated primitive list inside `MODERN_WHIMSY.md`. v1 keeps the build dead simple; PostCSS and minification can layer in when the surface grows.
